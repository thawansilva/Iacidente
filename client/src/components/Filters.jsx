...
export function Filters() {
  ...
  const { isFetching } = useContext(TrafficDataContext);

  const handleFormChange = (type, target) => {
    dispatch({ type: type.toUpperCase(), [`${type}`]: target });
  };

  return (
    <>
      <h3 className="text-2xl font-bold">Filtros</h3>
      <form onSubmit={handleFilterSubmit} className="text-indigo-50">
        ...
        <button className="w-full py-2 font-bold text-black duration-300 ease-in-out bg-blue-200 rounded-lg hover:bg-blue-400">
          {isFetching ? (
            <div role="status">
              <svg
                aria-hidden="true"
                className="w-8 h-8 mx-auto animate-spin dark:text-white fill-black"
                viewBox="0 0 100 101"
                fill="none"
